...
class NewUsersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create new_user" do
    assert_difference('NewUser.count') do
      post new_users_url, params: { new_user: { email: @new_user.email, name: @new_user.name } }
    end
  ...
  test "should update new_user" do
    patch new_user_url(@new_user), params: { new_user: { email: @new_user.email, name: @new_user.name } }
